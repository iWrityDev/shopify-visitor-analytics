...
app.get('/api/analytics/stats', (req, res) => {
  try {
    ...
    // Avg bounce time (bounced sessions with time)
    const bouncedWithTime = sessions.filter(s => s.bounced && s.totalTime > 0);
    const avgBounceTime = bouncedWithTime.length > 0
      ? Math.round(bouncedWithTime.reduce((sum, s) => sum + s.totalTime, 0) / bouncedWithTime.length)
      : 0;
    ...
    // Funnel: product views, cart, checkout, orders
